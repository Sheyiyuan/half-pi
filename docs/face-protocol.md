...
Face 与 Hand 使用不同的凭据表、token 类型和校验路径。Hub 以 `(peer_type, label)` 查询对应双秘密并完成 v2 proof 验证；同名 Face 与 Hand 仍是两个独立 principal。
...
- 使用临时目录和端口启动真实 Mind、Hand、Headless Face 和终端 REPL Face。
