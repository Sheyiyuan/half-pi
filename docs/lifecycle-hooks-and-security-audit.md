# 统一生命周期 Hook、安全审查与审计架构设计

> 状态：提案（2026-07-22），尚未实现。
>
> 本文描述 Half-Pi 当前 Chat、模型、工具、安全审批、事件投影和持久化审计的真实架构，解释现有问题，并定义后续统一生命周期内核的目标架构与迁移路径。本文不改变现有 Face wire protocol、RemoteRun 状态机或 Hand 最终守门原则。
...
这些能力不是错误的，问题在于它们没有共享一套明确的生命周期语义。当前的“Hook”“Event”“Audit”“Check”“Observer”分别由不同包定义，调用时机、错误处理、数据脱敏、顺序和持久化保证都不相同。新增入口或插件时，很难回答以下问题：
...
> Half-Pi 应建立统一的生命周期内核，但不能把安全拦截、数据变换、用户审批、持久化审计和插件通知合并成一个万能 EventBus。

目标架构使用同一套 lifecycle identity、阶段名称、顺序和脱敏规则，同时保留四种不同执行语义：
...
目标架构不是让所有箭头都经过 EventBus，而是让所有关键阶段都由 Lifecycle Coordinator 分配统一 identity，并按固定规则调用正确通道。
...
- `Sequence`：同一 trace 内单调递增，仅表达生命周期生成顺序；
- `OccurredAt`：UTC 时间。

Face connection `event_seq`、Gateway Envelope `Seq`、RemoteRun progress `seq` 和 lifecycle `Sequence` 保持独立，不能互相复用。它们分别解决连接线序、防重放、run 输出顺序和业务追踪顺序。
...
建议在 `half-pi-core/executor` 中引入不依赖 Mind internal 包的通用模型：
...
func (r *Runtime) Execute(context.Context, Invocation) Result
```

`half-pi-core` 只定义通用接口。Mind Authorizer 组合 session policy 和 Approval Broker；Hand Authorizer 组合 allow/deny、本机 `Tool.Check` 和 Approval proof 验证。
...
- 测试 helper 若需要绕过，使用仅测试构建可见的显式 fake Authorizer；
...
远程执行需要在审批前创建 run ID。不能简单地先运行通用 approval、再进入工具代码。建议把远程工具拆成：
...
这可以通过 ToolRuntime 的 `Prepare` 能力或 remote executor adapter 实现，但审批仍然由 Runtime 编排。目标是删除“工具自己完全拥有 confirm”的模糊语义，同时保留 run-bound approval。
...
建议把当前散落在 `Core.Chat()` 内的 system/Skill 拼接迁移为内置 Transformer。插件 Transformer 完成后，系统必须再次检查 token budget、消息角色合法性、tool schema 和最大请求大小。
...
不要把 `AfterModelResponse` 同时理解为“模型已返回”和“用户还没看到”。目标 API 使用 `model.response_received` 与 `assistant.before_deliver` 消除歧义。

### 13.4 流式输出策略

当前 Face streaming 会在完整响应生成前发送 delta。完整响应 Guard 无法撤回已发送文本。目标架构定义三种模式：
...
Hook 注册项如果声明需要完整响应或具有 `model.response.transform` capability，ChatRuntime 必须自动切换到 `buffered`。不能在 passthrough 模式下向用户承诺完整输出审查。

即使 buffered，provider delta 仍可生成内部 transient event，但只能发送给具有原始模型流 capability 的受信观察者，不能提前投影给普通 Face。

### 13.5 工具 progress 的同类问题

工具 stdout/stderr progress 也可能在最终结果 Transformer 前发送。需要结果安全过滤时：
...
当前四种 mode 把“风险分类”和“获得何种批准”混在同一个 `security.Policy.Check()` 结果里：
...
会话模型的 system prompt 当前要求它“自行判断风险”，但这只是提示词约定，不能构成安全控制。真正的 `trust` 策略仍直接放行非黑名单 Action；模型只有通过 `confirm: true` 才能主动加严。

目标是把风险分类从 mode 映射中拆出：确定性 Guard 负责拒绝不可接受的 Action；`review` mode 使用一个没有工具权限、没有会话写权限的独立 AI 请求，在已经冻结的 Action 上作出“自动通过”或“交给用户”的结构化判断。

#### 目标 mode

`trust` 应逐步改名为 `review` 或 `ai_review`。首个兼容版本可以继续读取和持久化旧值 `trust`，但运行时将其解释为 `review`，并在 UI、配置示例和新建 session 中只展示新名称。

| 目标 mode | 确定性 Guard 后的处理 | 用户审批 |
|---|---|---|
...
建议新增 append-only `security_decisions`，覆盖当前不完整的本地工具和消息/模型策略决策：
...
可靠插件通知和跨组件投影不应在数据库事务中直接调用插件。建议使用 `lifecycle_outbox`：
...
进入插件实现阶段前，应通过一个不执行动态代码的 fake consumer 验证：
...
以上验收通过后，[`plugin-architecture.md`](plugin-architecture.md) 才进入实施阶段。
...
├── lifecycle/              # Mind registry、scope、ordering、observer router
├── agentcore/              # ChatRuntime，编排 message/model/tool loop
├── policy/                 # session policy + Approval Broker adapter
├── audit/                  # security decision projector、outbox dispatcher
├── facegateway/            # lifecycle -> Face protocol projector
└── remoteexec/             # 保留 Authority/Registry，接入统一 Meta

modules/half-pi-hand/internal/
├── lifecycle/              # Hand node-local coordinator
├── policy/                 # allow/deny、Tool.Check、approval proof adapter
└── hand/                   # RPC admission、task manager、结果发送
```

是否建立单独 `policy/` 包可在实施时根据循环依赖调整；关键是 Authorizer 接口定义在 `half-pi-core`，Approval Broker 实现留在 Mind internal。
...
sequenceDiagram
    ...
    A->>A: permission + session policy + guards
...
- 接受本文或修订后的 ADR；
- 固定阶段名称、ID 关系、冻结点和失败策略；
- 明确 raw/redacted/digest 数据分类；
- 为现有行为补齐顺序测试，作为迁移基线；
- 冻结新增独立 `OnChange` 单槽 callback，除修 bug 外不再扩散。

完成标准：团队对“Event 不能阻止已发生事实”“Observer 不能充当 Guard”没有歧义。

### Phase 1：引入 lifecycle 基础包和兼容适配器

- 在 `half-pi-core/lifecycle` 定义 Meta、Phase、Outcome 和通用接口；
- Mind 创建 Coordinator 和多订阅者 Observer Router；
- 为 EventBus、Face Gateway、Approval/Run/Task observer 增加 adapter；
- 先双发新旧事件，通过测试比较顺序和内容；
- 不改变 wire protocol 和数据库 schema。

完成标准：现有 Face/TUI/Headless 行为和测试不变，新 lifecycle trace 可完整关联一次 Chat。

### Phase 2：收口本地 ToolRuntime

- 引入 Invocation、Authorizer 和 Runtime；
- 把 `prepareToolArgs`、schema 校验、digest、CheckAndConfirm 和 Execute 收进 Runtime；
- `Core.ExecuteTool`、LLM 工具调用和 REPL 入口改走 Runtime；
- 删除生产 `SkipChecks`；
- 增加 panic、nil result、timeout 和 cancellation 唯一终态；
- 增加本地 `security_decisions` 审计。

完成标准：生产代码无法取得一个绕过 Authorizer 的通用执行入口。

### Phase 3：接入远程执行与 Hand

- 将 `use_hand` 的 run-bound prepare/approval 纳入 Runtime；
- RemoteRun Registry 保持权威状态机，只增加 lifecycle Meta；
- Hand 引入自己的 Runtime/Authorizer adapter；
- 验证 Mind allow 不能覆盖 Hand deny；
- 背景 task、取消、重连和 lost 语义不变。

完成标准：远程闭环现有 race/E2E 全部通过，Mind 和 Hand lifecycle 可以通过 run ID 关联但裁决独立。

### Phase 4：接入 message/model lifecycle

- 在持久化用户消息前增加 `message.before_accept`；
- 将 Soul、Skill、compact、memory 等组装迁移到 `model.before_request` 内置 Transformer；
- 引入 `model.response_received` 和 `assistant.before_deliver`；
- 根据 Hook capability 自动选择 passthrough/buffered streaming；
- 旧 `ChatHooks` 仅作为 Face stream adapter，随后删除 context 单值。

完成标准：Face 和 REPL 入口共享同一输入/模型生命周期；启用完整输出 Guard 时不会提前流出 delta。

### Phase 5：可靠审计和 Outbox

- 增加 `security_decisions`；
- 增加低频可靠 lifecycle outbox；
- 建立重试、去重、dead-letter 和 retention；
- 明确 audit degraded 运维状态；
- 保留现有专用表和事务。

完成标准：进程在 mutation commit 后、可靠 consumer 收到事件前退出，重启后仍能重投；重复投递不会重复执行业务副作用。

### Phase 6：删除旧路径并冻结插件前置契约

- 删除 `ChatHooks` context 单值；
- 删除单槽 domain observer，改为 adapter subscription；
- 清理未使用或语义重复的 EventBus 类型；
- 固定 lifecycle wire fixture、redacted view、scope filter 和 fake consumer 验收；
- 更新 Face streaming、remote execution 和插件前置条件文档；
- 迁移完成后归档本文的实施部分，保留正式 contract。

完成标准：第 15 节的插件就绪验收全部通过，核心仓库尚不需要引入任何脚本或 WASM runtime；后续实施转入 [`plugin-architecture.md`](plugin-architecture.md)。
...
- observer event 顺序与 trace Sequence 一致。
...
- streaming redactor 跨 chunk 行为有专项测试；
...
- internal typed action 是使用分阶段接口，还是内部判别联合；
- buffered streaming 对 TUI 体验和延迟的影响；
- security decision audit 的 retention 和查询 API；
- audit degraded 后是仅告警、暂停敏感操作，还是进入只读模式。

这些选择不改变本文的核心边界。

## 24. 完成标准

本架构完成不是以“存在 Hook 接口”为准，而是同时满足：
...
- 关键审计可恢复、可关联、无秘密泄漏；
- 五模块 race、真实进程 E2E 和原生平台验收全部通过。

达到这些条件后，Half-Pi 才完成插件实现前的架构准备；具体运行时工作转入 [`plugin-architecture.md`](plugin-architecture.md)。
