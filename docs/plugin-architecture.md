...
- [x] 生命周期阶段、Meta、顺序和终态语义稳定；
- [x] ToolRuntime 成为唯一生产工具执行入口，生产路径不使用 `SkipChecks`；
- [x] Frozen Action、canonical digest、Approval 和实际执行参数完全绑定；
- [x] Guard、Transformer、Observer、Auditor 的失败语义已经分离；
- [x] raw、redacted、digest 三种数据视图由宿主生成；
- [x] SessionGroup、conversation、principal 和 node scope 可执行；
- [x] transactional outbox、可靠审计和重入限制可用。

Outbox dispatcher 当前不随 Mind 启动，因为还没有正式插件 consumer；插件实现必须提供 EventID 去重 consumer 后再接线，不能用空 consumer 确认待投递事件。
...
SessionGroup 是最低必要隔离边界。Skill frontmatter 已支持 `groups` allowlist，system prompt 使用 `IndexForGroup`，`view_skill` 使用 `GetForGroup`；空 allowlist 表示显式全局共享。插件必须复用同样的 group 语义，不能直接调用无 scope 的 `Store.Get`/`List` 暴露工作区私有内容。
...
1. [已完成] lifecycle 插件就绪验收和首版 wire fixture；
