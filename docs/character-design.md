...
3. **pass_soul 调用意愿低** — LLM 更倾向于直接写对话（零成本）而不是调工具（高成本）
...
→ LLM 调用 pass_soul(target="hanpai", reason="半拍更擅长缓存")
...
| pass_soul 调用 | 可选、工具调用成本高 | 自然——导演本就要调度演员 |
...
**不需要 `pass_soul` 工具。** 导演直接在 speak 里切换 soul 即可：
...
流程：speak 工具被调用 → AgentSession 记录当前 soul → 显示层在 text_delta 前打印 `[soul]` → 工具返回的 text 流式输出。

pass_soul 不输出标签——它只记录状态切换，在对话历史中注入一条 `[system]` 消息。

---

## 5. 工具执行

half-pi 的定位是"情绪价值优先"——工具执行如果污染了对话上下文，就本末倒置了。所以在 speak 和 pass_soul 之外的工具（bash/read/edit/write 等）应该由 subagent 隔离执行。这是 Phase 3 的内容。
...
- **N 轮上限**。连续 N 次 pass_soul 后等待用户输入。
- **用户始终可以打断**。
- **"情绪价值 >> 工程实用价值"** 是顶层哲学。
