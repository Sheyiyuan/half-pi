...
2. `/compact` 和 Skill 工作区集成继续按独立主线推进。

## 完成判定

当前阶段以 R0-R3 的 Linux/race 测试和 Windows 交叉编译验收通过为准；不据此宣称原生 Windows 已验收。Face Alpha P0-P4 runtime 已通过正式协议、race 测试和真实进程 E2E 验收。任何阶段仅在对应测试和文档能力声明与实际行为一致后标记完成。
