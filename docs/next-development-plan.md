...
Face Gateway、Conversation Actor、Chat 和异步审批 runtime 已在 P1-P3 落地；Headless/TUI/Web 客户端和进程级 E2E 继续按 P4 实施。独立 Face token/application key 和 Mind peer dispatcher 已随核心收口落地。
...
进行中：JSONL Headless Face、受限配置和正式 `registered` ready 输出已实现；真实 Mind/Hand/Face 进程 E2E 与人类终端 Face 尚待完成。
