...
- Headless Agent Face 和真实进程级 E2E。
...
Face Gateway、Conversation Actor、Chat 和异步审批 runtime 已在 P1-P3 落地；Headless JSONL 与人类终端客户端已在 P4 落地，真实进程级 E2E 继续实施。独立 Face token/application key 和 Mind peer dispatcher 已随核心收口落地。

## 已完成：Face 核心收口

2026-07-18 已补齐持久化后台 task 协议、独立 Face/Hand credentials、Mind 类型分流、复合 peer identity、四步挑战握手和注册后业务 payload 强制加密。旧 `hand_tokens` 与旧三步握手 fail closed；Hand 必须重新创建凭据并配置 application key。实施规格和验收矩阵见 [`face-core-closure-plan.md`](face-core-closure-plan.md)。该范围不包含 Face Gateway command routing、Conversation Actor、Chat runtime 或客户端；Windows 原生 ACL 和取消行为仍按发布环境单独验收。
...
2026-07-19 已完成进程级 Approval Broker、Face/REPL 统一裁决、SQLite 脱敏审计、pending 快照、审批事件、Authority run cancel、TaskService task cancel 和完整 run 状态投影。加密集成测试已覆盖 Face → approval → `use_hand`、参数篡改 digest mismatch、run/task cancel 与状态落库；P3 已完成，下一阶段为 P4。
...
进行中：JSONL Headless Face、受限配置、正式 `registered` ready 输出和共用协议的人类终端 Face 已实现；真实 Mind/Hand/Face 进程 E2E 尚待完成。
...
P0-P3 已完成：wire contract、凭据、加密会话、类型分流、Conversation Actor、Gateway、Chat、异步审批和 run/task cancel 均已落地。P4 Headless Face 与进程级 E2E 是当前 Face Alpha 主线。
...
2. 实施 Face Alpha Runtime P4 Headless Face 与进程级 E2E。
3. `/compact` 和 Skill 工作区集成继续按独立主线推进。

## 完成判定

当前阶段以 R0-R3 的 Linux/race 测试和 Windows 交叉编译验收通过为准；不据此宣称原生 Windows 已验收。Face Alpha 仍以 P0-P4 runtime 全部验收为准，其中 P0-P3 已完成、P4 尚未完成。任何阶段仅在对应测试和文档能力声明与实际行为一致后标记完成。
