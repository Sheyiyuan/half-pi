...
> 状态：核心架构已实现（2026-07-22）。Phase 0-6 的代码前置工作已经落地；动态插件 runtime 不属于本文范围，仍由 [`plugin-architecture.md`](../plugin-architecture.md) 规划。
>
...
Soul、会话 mode、按 SessionGroup 过滤后的 Skill 索引和 `/compact` 上下文摘要已经迁移为内置 `model.before_request` Transformer。memory 尚未实现，但后续只能通过同一阶段接入。所有 Transformer 完成后，系统会重新检查消息数量与角色、UTF-8、tool schema 可序列化性和总请求大小。
