...
本文是 Mind 本地管理面的实施设计，状态为 **待实现**。它解决当前 Face/Hand 凭据只能通过 Mind REPL 创建和撤销、默认 service mode 缺少管理入口的问题。

本文不改变 Face/Hand wire protocol，不引入远程管理 HTTP API，也不改变 Mind 默认以 service mode 启动的行为。现有 REPL 命令继续保留，但改为复用本文定义的管理服务。
...
- 首次配置 TUI/Headless Face 必须临时启动 REPL。
...
half-pi-mind peers [--format table|json]
```
...
Windows named pipe 使用仅允许当前用户 SID 和 SYSTEM 的 DACL。实现放在 `_windows.go` 文件中，可使用专门的 named pipe 库；不得回退为无认证的 loopback TCP 端口。
...
凭据 add/remove 是安全边界变更，在线、离线和 REPL 必须产生一致审计。新增表建议：
...
stdout 只承载结果；stderr 只承载诊断。JSON 模式下 stdout 恰好输出一个 JSON 对象，不混入提示文本。
...
- Unix `flock` 和 Windows `LockFileEx` 原生测试通过。
...
```text
Mind 未运行 → offline face add → 启动 Mind → TUI Face 认证成功
...
- 在原生 Windows 验收 named pipe、ACL 和状态锁。

## 完成标准

满足以下条件后，可移除“创建 Face/Hand 凭据必须进入 REPL”的限制：
...
- Unix race E2E 与原生 Windows IPC/锁/ACL 验收通过。
