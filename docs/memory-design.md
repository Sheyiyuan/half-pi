...
half-pi 的记忆系统需要解决三个核心问题：**分层筛选**（哪些该同步、哪些该注入）、**冲突处理**（多设备写同一记忆）、**自我迭代**（Agent 自己整理记忆）。

---

## 1. 为什么是 Markdown + YAML（而非数据库）
...
- SQLite+vec 的检索能力完美匹配「语义搜索」需求（远期 Phase 5）
...
<memory id="2026-05-21-pref-editor" weight="0.85">
社老师是 Vim 用户。在帮他配置编辑器或写快捷键文档时
默认使用 Vim 风格。
</memory>
...
超过预算的部分截断，末尾注明 `... (3 more memories omitted, use memory:search to retrieve)`。
...
Phase 1-2 覆盖「分层存储 + 权重筛选」，是记忆系统的最小可行版本。
Phase 3-4 实现 Agent 自迭代整理。
Phase 5 是向量检索的远期升级，接口已预留。
