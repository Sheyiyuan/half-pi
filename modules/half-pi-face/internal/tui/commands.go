...
func defaultCommandSpecs() []CommandSpec {
	...
	return []CommandSpec{
		...
		{Name: "create", Aliases: []string{"new"}, Description: "Create a named conversation", Args: []ArgumentSpec{{Name: "name", Required: true, Variadic: true}}, Scopes: []protocol.FaceScope{protocol.FaceScopeSessionsWrite}, Execute: commandCreate},
		{Name: "open", Aliases: []string{"resume"}, Description: "Open a conversation", Args: []ArgumentSpec{conversationArg}, Scopes: []protocol.FaceScope{protocol.FaceScopeSessionsRead}, Execute: commandOpen},
